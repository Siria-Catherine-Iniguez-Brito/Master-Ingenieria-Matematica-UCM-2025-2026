...
!
!    Input/output, real ( kind = rk8 ) C(N), contains the subdiagonal of the
!    tridiagonal matrix in entries C(2:N).  On output, C is destroyed.
!
!    Input/output, real ( kind = rk8 ) D(N).  On input, the diagonal of the
!    matrix.  On output, D is destroyed.
!
!    Input/output, real ( kind = rk8 ) E(N), contains the superdiagonal of the
!    tridiagonal matrix in entries E(1:N-1).  On output E is destroyed.
!
!    Input/output, real ( kind = rk8 ) B(N).  On input, the right hand side.
...
!  Author:
!
!    Original Fortran77 version by Robert Piessens, Elise de Doncker.
!    This version by John Burkardt.
!
!  Parameters:
!
...
!
!      epsabs - real ( kind = rk8 )
!               absolute accuracy requested
!      epsrel - real ( kind = rk8 )
...
!
!   on return
!      result - real ( kind = rk8 )
!               approximation to the integral
!
!      abserr - real ( kind = rk8 )
...
!
  ...
  real ( kind = rk8 ) a,abserr,alist,area,area1,area12,area2,a1,a2,b, &
...
!
  ...
  if ( epsabs <= 0.0D+00 .and. &
    epsrel < max ( 0.5D+02 * epmach, 0.5d-28 ) ) then
    ier = 6
    return
  end if
!
...
!
  ...
  if ( abserr <= 0.5D+02 * epmach * defabs .and. &
    errbnd < abserr ) then
    ier = 2
  end if
...
!
  ...
  real ( kind = rk8 ) b
  real ( kind = rk8 ) epsabs
  real ( kind = rk8 ) epsrel
  ...
  integer l3
  integer neval
  real ( kind = rk8 ) result
  real ( kind = rk8 ) work(lenw)
!
...
!
  ...
  real ( kind = rk8 ) abseps,abserr,alist,area,area1,area12,area2,a1, &
  ...
  integer id,ier,ierro,inf,iord,iroff1,iroff2, &
...
!
  ...
  real ( kind = rk8 ) abserr,bound,epsabs,epsrel,f,result,work
...
!
  ...
  real ( kind = rk8 ) a,abseps,abserr,alist,area,area1,area12,area2,a1, &
  ...
  integer i,id,ier,ierro,ind1,ind2,iord,ip1, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,b,epsabs,epsrel,f,points,result,work
...
!
  ...
  integer id,ier,ierro,iord,iroff1,iroff2,iroff3,jupbnd, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,b,epsabs,epsrel,f,result,work
...
!
  ...
  real ( kind = rk8 ) a,aa,abserr,alist,area,area1,area12,area2,a1,a2, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,b,c,epsabs,epsrel,f,result,work
  integer ier,iwork,last,lenw,limit,lvl,l1,l2,l3,neval
  dimension iwork(limit),work(lenw)
...
!
!      alist, blist, rlist, elist - real ( kind = rk8 )
...
!
  ...
  real ( kind = rk8 ) a,abseps,abserr,alist,blist,chebmo,correc,cycle, &
  ...
  integer limlst
  integer ll
  integer maxp1,momcom,nev,neval,nnlog,nres,numrl2
...
!
  ...
  integer iroff3
  integer jupbnd
  integer k,ksgn,ktmin,last,limit,maxerr,maxp1,momcom,nev,neval, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,b,epsabs,epsrel,f,omega,result,work
  integer ier,integr,iwork,last,limit,lenw,leniw,lvl
  integer l1
  integer l2
  integer l3
  integer l4
  integer maxp1,momcom,neval
...
!
  ...
  real ( kind = rk8 ) a,abserr,alfa,alist,area,area1,area12,area2,a1, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,alfa,b,beta,epsabs,epsrel,f,result,work
...
!
  ...
  real ( kind = rk8 ) a,abserr,ak22,amom0,amom1,amom2,b,c,cc,centr, &
...
!              number of integrand evaluations
!
!     resabs - real ( kind = rk8 )
!              approximation to the integral j
!
!     resasc - real ( kind = rk8 )
...
!
  ...
  real ( kind = rk8 ) a,abserr,ac,an,an2,as,asap,ass,b,centr,chebmo, &
...
!
  ...
  real ( kind = rk8 ) a,abserr,alfa,b,beta,bl,br,centr,cheb12,cheb24, &
...
!
  ...
  real ( kind = rk8 ) alam,alam1,alam2,cheb12,cheb24,fval,part1,part2, &
    part3,v,x
  integer i,j
...
!
  ...
  real ( kind = rk8 ) abserr,delta1,delta2,delta3, &
...
!
  ...
  real ( kind = rk8 ) a,absc,abserr,b,centr,dhlgth, &
...
!
  ...
  real ( kind = rk8 ) a,absc,absc1,absc2,abserr,b,boun,centr,dinf, &
...
!  Discussion:
!
!    Recently a bug was discovered in subroutines QK15W, DQK15W, 
!    which are Quadpack routines included in SLATEC.  In these 
!    routines, the DATA initialization of WG(3) should be changed from
!      0.3818300505051889E+0 and 0.3818300505051889D+00
!    to
!      0.3818300505051189E+0 and 0.3818300505051189D+00
!    respectively.
!
!    DONE, JVB, 16 November 2023.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  real ( kind = rk8 ) a,absc,absc1,absc2,abserr,b,centr,dhlgth, &
    epmach,f,fc,fsum,fval1,fval2,fv1,fv2,hlgth, &
...
!
  ...
  real ( kind = rk8 ) a,dabsc,abserr,b,centr,dhlgth, &
    epmach,f,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc, &
    resg,resk,reskh,result,uflow,wg,wgk,xgk
  integer j,jtw,jtwm1
  external f
...
!
  ...
  real ( kind = rk8 ) alfa,alfp1,alfp2,an,anm1,beta,betp1,betp2,ralf, &
    rbet,rg,rh,ri,rj
...
!
  ...
  real ( kind = rk8 ) elist,ermax,errmax,errmin
...
!
  ...
  real ( kind = rk8 ) dqwgtc
  real ( kind = rk8 ) c,p2,p3,p4,x
  integer kp
...
!*****************************************************************************80
!
!! dqwgtf() defines the weight function used by DQC25F.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  real ( kind = rk8 ) a
  real ( kind = rk8 ) dqwgts
  real ( kind = rk8 ) alfa,b,beta,bmx,x,xma
  integer integr
