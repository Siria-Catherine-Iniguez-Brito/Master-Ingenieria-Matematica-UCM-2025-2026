...
!
!    Input, real ( KIND(0d0) ) X(N), the point at which the jacobian is to be
!    evaluated.
!
!    Input, real ( KIND(0d0) ) FVEC(M), is used only when MODE = 2.  
!    In that case, it should contain the function values at X.
!
!    Input, real ( KIND(0d0) ) FJAC(LDFJAC,N), an M by N array.  When MODE = 2,
...
!
!    Output, real ( KIND(0d0) ) XP(N), on output with MODE = 1, is a neighboring
!    point of X, at which the function is to be evaluated.
!
!    Input, real ( KIND(0d0) ) FVECP(M), on input with MODE = 2, is the function
!    value at XP.
!
!    Input, integer MODE, should be set to 1 on the first call, and
!    2 on the second.
!
!    Output, real ( KIND(0d0) ) ERR(M).  On output when MODE = 2, ERR contains
...
!
  ...
  real ( KIND(0d0) ) eps
  real ( KIND(0d0) ) epsf
  real ( KIND(0d0) ) epslog
  real ( KIND(0d0) ) epsmch
  real ( KIND(0d0) ) err(m)
  real ( KIND(0d0) ) fjac(ldfjac,n)
  real ( KIND(0d0) ) fvec(m)
  real ( KIND(0d0) ) fvecp(m)
  integer i
  integer j
  integer mode
  real ( KIND(0d0) ) temp
...
!  Parameters:
!
!    Input/output, real ( KIND(0d0) ) X, Y.  On output, the values of X and
...
!
!    Input, real ( KIND(0d0) ) R(LR), the upper triangular matrix R stored
!    by rows.
!
!    Input, integer LR, the size of the R array, which must be no less
!    than (N*(N+1))/2.
!
!    Input, real ( KIND(0d0) ) DIAG(N), the diagonal elements of the matrix D.
!
!    Input, real ( KIND(0d0) ) QTB(N), the first N elements of the vector Q'* B.
!
!    Input, real ( KIND(0d0) ) DELTA, is a positive upper bound on the 
!    euclidean norm of D*X(1:N).
!
!    Output, real ( KIND(0d0) ) X(N), the desired convex combination of the
...
!
!    Input, real ( KIND(0d0) ) X(N), the vector whose norm is desired.
!
!    Output, real ( KIND(0d0) ) ENORM, the Euclidean norm of the vector.
!
  implicit none
...
!    Input, real ( KIND(0d0) ) X(N), the vector whose norm is desired.
!
!    Output, real ( KIND(0d0) ) ENORM2, the Euclidean norm of the vector.
!
  implicit none

  integer n

  real ( KIND(0d0) ) agiant
  real ( KIND(0d0) ) enorm2
  integer i
  real ( KIND(0d0) ) rdwarf
  real ( KIND(0d0) ) rgiant
  real ( KIND(0d0) ) s1
  real ( KIND(0d0) ) s2
  real ( KIND(0d0) ) s3
  real ( KIND(0d0) ) x(n)
...
!
!    Input, real ( KIND(0d0) ) X(N), the point where the jacobian is evaluated.
!
!    Input, real ( KIND(0d0) ) FVEC(N), the functions evaluated at X.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), the N by N approximate 
!    jacobian matrix.
!
...
!
!    Input, real ( KIND(0d0) ) EPSFCN, is used in determining a suitable step
...
!
  ...
  real ( KIND(0d0) ) eps
  real ( KIND(0d0) ) epsfcn
  real ( KIND(0d0) ) epsmch
  external fcn
  real ( KIND(0d0) ) fjac(ldfjac,n)
  real ( KIND(0d0) ) fvec(n)
  real ( KIND(0d0) ) h
  integer i
  integer iflag
  integer j
...
!    Input, real ( KIND(0d0) ) X(N), the point where the jacobian is evaluated.
!
!    Input, real ( KIND(0d0) ) FVEC(M), the functions evaluated at X.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), the M by N approximate 
!    jacobian matrix.
!
!    Input, integer LDFJAC, the leading dimension of FJAC, which must
!    not be less than M.
...
!    to the calling program, with the value of IFLAG.
!
!    Input, real ( KIND(0d0) ) EPSFCN, is used in determining a suitable 
...
!    Input, integer N, the number of functions and variables.
!
!    Input/output, real ( KIND(0d0) ) X(N).  On input, X must contain an initial 
!    estimate of the solution vector.  On output X contains the final 
!    estimate of the solution vector.
!
!    Output, real ( KIND(0d0) ) FVEC(N), the functions evaluated at the output X.
!
!    Input, real ( KIND(0d0) ) XTOL.  Termination occurs when the relative error
...
!    precision.
!
!    Input/output, real ( KIND(0d0) ) DIAG(N).  If MODE = 1, then DIAG is set
...
!
!    Input, real ( KIND(0d0) ) FACTOR, determines the initial step bound.  This
...
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), an N by N array which contains
...
!
!    Output, real ( KIND(0d0) ) R(LR), the upper triangular matrix produced by
!    the QR factorization of the final approximate jacobian, stored rowwise.
!
!    Input, integer LR, the size of the R array, which must be no less
!    than (N*(N+1))/2.
!
!    Output, real ( KIND(0d0) ) QTF(N), contains the vector Q'*FVEC.
!
  implicit none
...
!    Output, real ( KIND(0d0) ) FVEC(N), the functions evaluated at the output X.
!
!    Input, real ( KIND(0d0) ) TOL.  Termination occurs when the algorithm
...
!    Output, real ( KIND(0d0) ) FVEC(N), the functions evaluated at the output X.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), an N by N matrix, containing
...
!
!    Output, real ( KIND(0d0) ) R(LR), the upper triangular matrix produced 
...
!    estimate of the solution vector.
!
!    Output, real ( KIND(0d0) ) FVEC(M), the functions evaluated at the output X.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), an M by N array.  The upper 
...
!    LDFJAC must be at least M.
!
!    Input, real ( KIND(0d0) ) FTOL.  Termination occurs when both the actual
...
!    nonnegative.
!
!    Input, real ( KIND(0d0) ) GTOL.  Termination occurs when the cosine of the
...
!
!    Output, real ( KIND(0d0) ) QTF(N), contains the first N elements of Q'*FVEC.
!
  implicit none
...
!    to FCN is at least MAXFEV by the end of an iteration.
!
!    Input, real ( KIND(0d0) ) EPSFCN, is used in determining a suitable step length for 
...
!    2, scaling is specified by the input DIAG vector. 
!
!    Input, real ( KIND(0d0) ) FACTOR, determines the initial step bound.  This bound is 
...
!    Output, integer NFEV, the number of calls to FCN.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), an M by N array.  The upper N by N submatrix
...
!
!    Output, real ( KIND(0d0) ) QTF(N), the first N elements of Q'*FVEC.
!
  implicit none
...
!
!    Input/output, real ( KIND(0d0) ) R(LDR,N),the N by N matrix.  The full 
...
!    Input, real ( KIND(0d0) ) QTB(N), the first N elements of the vector Q'*B.
!
!    Input, real ( KIND(0d0) ) DELTA, an upper bound on the euclidean norm of D*X.  
!    DELTA should be positive.
!
!    Input/output, real ( KIND(0d0) ) PAR.  On input an initial estimate of the 
!    Levenberg-Marquardt parameter.  On output the final estimate.  
!    PAR should be nonnegative.
!
!    Output, real ( KIND(0d0) ) X(N), the least squares solution of the system A*X = B, 
!    sqrt(PAR)*D*X = 0, for the output value of PAR.
!
!    Output, real ( KIND(0d0) ) SDIAG(N), the diagonal elements of the upper triangular 
!    matrix S.
!
  implicit none
  ...
  real ( KIND(0d0) ) qnorm
  real ( KIND(0d0) ) qtb(n)
  real ( KIND(0d0) ) r(ldr,n)
  real ( KIND(0d0) ) sdiag(n)
  real ( KIND(0d0) ) sum2
  real ( KIND(0d0) ) temp
  real ( KIND(0d0) ) wa1(n)
  real ( KIND(0d0) ) wa2(n)
...
!    Output, real ( KIND(0d0) ) FVEC(M), the functions evaluated at the output X.
!
!    Output, real ( KIND(0d0) ) FJAC(LDFJAC,N), an N by N array.  The upper
...
!    LDFJAC must be at least N.
!
!    Input, real ( KIND(0d0) ) FTOL.  Termination occurs when both the actual and 
!    predicted relative reductions in the sum of squares are at most FTOL.
!    Therefore, FTOL measures the relative error desired in the sum of 
!    squares.  FTOL should be nonnegative.
!
!    Input, real ( KIND(0d0) ) XTOL.  Termination occurs when the relative error between 
!    two consecutive iterates is at most XTOL.  XTOL should be nonnegative.
!
!    Input, real ( KIND(0d0) ) GTOL. termination occurs when the cosine of the angle 
...
!    to FCN with IFLAG = 1 is at least MAXFEV by the end of an iteration.
!
!    Input/output, real ( KIND(0d0) ) DIAG(N).  If MODE = 1, then DIAG is set internally.
...
!    LDFJAC must be at least N.
!
!    Input, real ( KIND(0d0) ) TOL. Termination occurs when the algorithm estimates 
...
!    of columns of A.
!
!    Input/output, real ( KIND(0d0) ) Q(LDQ,M).  Q is an M by M array.
...
!
!    Input/output, real ( KIND(0d0) ) A(LDA,N), the M by N array.
...
!    pivoting is used.
!
!    Output, real ( KIND(0d0) ) RDIAG(N), contains the diagonal elements of R.
!
!    Output, real ( KIND(0d0) ) ACNORM(N), the norms of the corresponding 
...
!
  ...
  real ( KIND(0d0) ) a(lda,n)
  real ( KIND(0d0) ) acnorm(n)
  real ( KIND(0d0) ) ajnorm
  real ( KIND(0d0) ) enorm
  real ( KIND(0d0) ) epsmch
  integer i
  integer ipvt(lipvt)
...
!    Input, integer N, the order of R.
!
!    Input/output, real ( KIND(0d0) ) R(LDR,N), the N by N matrix.
...
!    Input, real ( KIND(0d0) ) QTB(N), the first N elements of the vector Q'*B.
!
!    Output, real ( KIND(0d0) ) X(N), the least squares solution.
!
!    Output, real ( KIND(0d0) ) SDIAG(N), the diagonal elements of the upper
!    triangular matrix S.
!
  implicit none
...
!    Input, integer N, the order of R.
!
!    Input/output, real ( KIND(0d0) ) R(LDR,N).  On input the upper triangular
...
!
!    Input, real ( KIND(0d0) ) W(N), the row vector to be added to R.
!
!    Input/output, real ( KIND(0d0) ) B(N).  On input, the first N elements
!    of the vector C.  On output the first N elements of the vector Q'*C.
!
!    Input/output, real ( KIND(0d0) ) ALPHA.  On input, the (N+1)-st element 
!    of the vector C.  On output the (N+1)-st element of the vector Q'*C.
!
!    Output, real ( KIND(0d0) ) C(N), S(N), the cosines and sines of the
...
!    be less than M.
!
!    Input, real ( KIND(0d0) ) V(N), W(N), contain the information necessary 
...
!
!    Input/output, real ( KIND(0d0) ) S(LS).  On input, the lower trapezoidal
...
!    (N*(2*M-N+1))/2.
!
!    Input, real ( KIND(0d0) ) U(M), the U vector.
!
!    Input/output, real ( KIND(0d0) ) V(N).  On input, V must contain the vector V.
!    On output V contains the information necessary to recover the Givens 
!    rotations GV described above.
!
!    Output, real ( KIND(0d0) ) W(M), contains information necessary to 
